...
//获取手机相关信息
    ...
    public void saveRemainTime(long countdownTime, boolean isStop) {
        ...
        editor.putLong(AlarmClockCommon.COUNTDOWN_TIME, countdownTime);
        editor.putBoolean(AlarmClockCommon.IS_STOP, isStop);
        editor.apply();
    }
...
//        cancelTimer();
    ...
    private void process(int h, int m, int s, boolean isStop) {
        ...
        if (!isStop) {
            // 开启计时
            if (start()) {
               OttoBus.getInstance().post(new TimerStartEvent());
            }
        }
    }
/*
